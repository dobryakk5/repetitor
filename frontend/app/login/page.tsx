  ...
  return (
    <main className="min-h-screen bg-[#f5f4f0] px-4 py-10 text-[#1a1a18]">
      <section className="mx-auto max-w-md rounded-[10px] border border-[#e0ddd6] bg-white p-6 shadow-sm">
        ...
        <h1 className="mt-1 text-[26px] font-semibold">Вход</h1>
        ...
        <button onClick={() => void submit()} disabled={loading} className="mt-5 h-10 w-full rounded-[8px] bg-[#1a1a18] px-5 text-[13px] font-medium text-white disabled:opacity-50">{loading ? 'Вход...' : 'Войти'}</button>
        <p className="mt-4 text-center text-[13px] text-[#73726c]">Нет аккаунта? <Link href="/register" className="font-medium text-[#1a1a18]">Зарегистрироваться</Link></p>
      </section>
